...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //determineDx and sizeSwitcher were incorporated into this function and FSL was dealth with.
    switch(size) {
      case "1":
        newwidth = 25;
        break;
      case "2":
        newwidth = 33.3;   //determineDx is removed as the function can work without it.
        break;
      case "3":
        newwidth = 50;
        break;
    ...
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer"); //Brought out of the loop to stop multiple iterations
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  //numberOfEntries variable removed to reduce memory usage.
  var sum = 0;
  for (var i = times.length - 1; i > times.length - 11; i--) {
...
function updatePositions() {
  ...
  storedVal = document.body.scrollTop / 1250; //Brought this out of the for loop to have this computation only once

  for (var j = 0; j < 5 ; j++){
    phase[j] = Math.sin(storedVal + j); //Phase only has 5 possible values as calculations are being done modulo 5. Computation 200 times in the loop is inefficient.
  }
...
//I have added requestAnimation frame on both functions because it uses animation functions .style.top and the updatePositions func contains transform
